...
describe('M0 core: independent domain contract', () => {
  ...
  it('builds scene snapshots from the same content identity and isolates geometry copies', () => {
    ...
    expect(loaded.scene.roads).toEqual([
      { id: 'road_AB', name: map.roads.road_AB!.name, fromNodeId: 'node_A', toNodeId: 'node_B', points: [[0, 0, 0], [100, 0, 0]], lengthM: 100, direction: map.roads.road_AB!.direction, widthM: { state: 'unknown' } },
    ]);
