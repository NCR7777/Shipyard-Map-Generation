...
网格吸附、节点吸附、新建设施/区域类型及两种整体移动策略，现在随 `projectId` 保存在现有 IndexedDB `editorStates` 中。配置变化单独自动保存（600ms 防抖）；“保存工程”或 Ctrl+S 立即保存完整视窗与绘图配置。仅改配置不会修改地图 JSON、revision、摘要或撤销历史。调整视窗后再保存也会保留全部配置。

“恢复绘图默认配置”仅重置这六项设置。旧 camera-only 记录自动补默认值；损坏配置提示后仍恢复有效地图，保存失败明确显示未保存。刷新回到选择工具，矩形默认自动识别，不恢复临时自由变形模式、弹窗或未完成输入。字段与失败行为见[保存契约](docs/M11_PERSISTENCE_CONTRACT.md)。
