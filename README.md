...
选建筑“添加入口”，默认先放置节点、稍后接路。画布高亮建筑外边界，鼠标靠近时绿色圆点显示吸附后的入口位置；确认仅创建入口与独立节点。之后用“道路”从该点画出连接；“独立节点”也可先放普通节点。作业点可先放在建筑或区域内，默认用途为“其他”，到达方式留待补充。需要立即接路时选择“创建时明确接路或到达方式”，保留原接路和转向预览。
