...
“保存工程”、Ctrl+S 和菜单进入同一流程。存在未应用输入时，可选“应用后保存”“仅保存已提交地图”或取消；无效输入保留，未完成绘图不会自动闭合。有授权文件关联时写回该文件并维护浏览器恢复；无关联时选择保存目标。下载只表示浏览器已发起下载，不等于确认文件写回。新导入会适应地图，刷新已保存工程恢复其相机。
