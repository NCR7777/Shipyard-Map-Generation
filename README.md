...
“保存工程”、Ctrl+S 和菜单进入同一流程。支持授权的浏览器通过“导入 JSON”关联原文件，后续保存直接写回，刷新和项目切换后仍保留关联。旧的浏览器副本首次选择“保存到文件 → 选择原文件并写回”，核对并确认一次即可；“导入 JSON 副本”和“保存为新文件”保留独立副本语义。有未应用输入时，可选应用后保存、仅保存已提交地图或取消；外部修改仍需明确处理冲突。不支持文件授权的浏览器使用下载，不能声称已覆盖原文件。详见 [原文件保存与编辑修复](docs/SV01_SAVE_EDIT_FIX.md)。
