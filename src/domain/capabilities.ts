...
export function mapCapabilities(map: YardMap): CapabilityReport {
  ...
  for (const key of ['junctions', 'movements', 'resources', 'assets', 'backgroundLayers'] as const) {
    if (Object.keys(map[key]).length > 0) { unrendered.push(key); reasons.push(`M2A 不支持编辑 ${key}，原始数据完整保留。`); }
  }
  if (map.siteBoundary) { unrendered.push('siteBoundary'); reasons.push('M2A 不支持厂区边界编辑。'); }
  if (map.coordinateFrame.geographicAnchor) reasons.push('地理锚定转换尚未实现。');
  if (Object.values(map.roads).some(r => r.corridorPolygon)) {
    unrendered.push('roads.corridorPolygon'); reasons.push('存在人工道路边界；M2A 不支持其同步编辑。');
  }
