...
export function toSceneSnapshot(map: YardMap): SceneSnapshot {
  ...
  return {
    ...
    bounds: geometryBounds([...nodes.map(n => n.position), ...roadBounds.flatMap(road => road.bounds ? [road.bounds.min, road.bounds.max] : []), ...[...facilities, ...zones].flatMap(entity => [entity.boundary.outer, ...entity.boundary.holes].flat())]),
