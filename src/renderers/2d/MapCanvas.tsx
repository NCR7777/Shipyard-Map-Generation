...
export type Tool = 'select' | 'node' | 'road' | 'pan';
...
interface Props {
  ...
  readonly: boolean;
  selection: Selection;
  onSelect: (kind: 'nodes' | 'roads', id: string, additive: boolean) => void;
...
export function MapCanvas(props: Props) {
  ...
  const selectedNodeIds = useMemo(() => {
    ...
    return ids;
  }, [props.selection, props.scene.roads]);
  ...
  function handleStageClick(event: KonvaEventObject<MouseEvent>) {
    ...
    const world = screenToWorld(screen, props.camera);
    if (props.tool === 'node' && !props.readonly) props.onAddNode(world);
  ...
  return <div ref={wrapper} className={`canvas-surface tool-${props.tool}`} data-testid="map-canvas" aria-label="米制地图画布">
    <Stage ref={stage} width={size.width} height={size.height}
      ...
      onMouseMove={() => {
        ...
        const world = screenToWorld(point, props.camera); props.onCursor(world); setRubberEnd(world);
      }}
      onWheel={event => {
      ...
      <Layer>
        {visibleRoads.map(road => <Line key={road.id} name="road" points={points(road)} stroke={props.selection.roads.includes(road.id) ? '#e08128' : '#216b88'} strokeWidth={props.selection.roads.includes(road.id) ? 5 : 3} hitStrokeWidth={14} lineCap="round" lineJoin="round"
          onClick={event => {
            ...
            else if (!props.readonly && (props.tool === 'node' || props.tool === 'road')) {
              const screen = pointer(); if (!screen) return; event.cancelBubble = true;
              const world = screenToWorld(screen, props.camera);
              if (props.tool === 'node') props.onAddNode(world); else props.onRoadPoint(world);
            }
          }} />)}
        {visibleNodes.map(node => {
          ...
          return <Circle _useStrictMode key={node.id} x={x} y={y} radius={selected ? 6.5 : 5} fill={selected ? '#e08128' : '#ffffff'} stroke={selected ? '#9a4c0d' : '#216b88'} strokeWidth={2} hitStrokeWidth={12}
            ...
            onDragMove={event => {
              ...
              const value = screenToWorld([event.target.x(), event.target.y()], props.camera, node.position[2]);
