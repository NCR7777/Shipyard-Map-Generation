...
type SpatialEvent = KonvaEventObject<MouseEvent | DragEvent>;
function spatialTarget(event: SpatialEvent, props: SpatialLayerProps) {
  const kind = event.currentTarget.getAttr('entityKind') as 'facilities' | 'zones' | 'accessPoints' | 'servicePoints';
  const id = event.currentTarget.getAttr('entityId') as string;
  if (kind === 'facilities' || kind === 'zones') {
    const item = props.scene[kind].find(item => item.id === id);
    return item ? { kind, id, origin: item.boundary.outer[0], nodeId: undefined } : null;
  }
  if (kind === 'accessPoints' || kind === 'servicePoints') {
    const item = props.scene[kind].find(item => item.id === id);
    return item ? { kind, id, origin: item.position, nodeId: item.nodeId } : null;
  }
  return null;
}
/** Shared subscriptions; the current callback still reads every live edit guard and origin. */
function useSpatialEvents(props: SpatialLayerProps) {
  const onMouseEnter = useCurrentCallback((event: KonvaEventObject<MouseEvent>) => {
    const target = spatialTarget(event, props);
    if (target) props.onHover?.(target.kind + '/' + target.id);
  });
  const onMouseLeave = useCurrentCallback(() => props.onHover?.(null));
  const onMouseDown = useCurrentCallback((event: KonvaEventObject<MouseEvent>) => {
    const target = spatialTarget(event, props);
    if (target && event.evt.button === 0 && props.selecting && (!props.selection[target.kind]?.includes(target.id) || event.evt.shiftKey))
      props.onSelect(target.kind, target.id, event.evt.shiftKey);
  });
  const onClick = useCurrentCallback((event: KonvaEventObject<MouseEvent>) => {
    if (event.evt.button !== 0) return;
    const target = spatialTarget(event, props); if (!target) return;
  ...
  const onDragStart = useCurrentCallback((event: KonvaEventObject<DragEvent>) => {
    const target = spatialTarget(event, props); if (target) props.onDragStart([...target.origin]);
  });
  const onDragMove = useCurrentCallback((event: KonvaEventObject<DragEvent>) => {
...
export function DeclaredLayer({ items, camera, hiddenTypes, visibleKeys, onHover, selectedKey, onSelect, selection, previewDelta, movingJunctionIds, backdrop = false }: {
  ...
  const onPick = useCurrentCallback((event: KonvaEventObject<MouseEvent>) => {
    if (event.evt.button !== 0) return;
    const item = items.find(item => item.key === event.currentTarget.getAttr('sceneKey'));
    if (item) { event.cancelBubble = true; onSelect(item); }
  });
  return <>{items.filter(item => !hiddenTypes.includes(item.kind) && (!visibleKeys || visibleKeys.has(item.key)) && (backdrop ? item.kind === 'siteBoundary'
    ...
    return <Group key={item.key} name={'declared-' + item.kind} sceneKey={item.key} onMouseEnter={onEnter} onMouseLeave={onLeave} onClick={onPick}>
      {item.polygons.map((polygon, index) => {
        ...
        return <Shape key={'polygon-' + index} stroke={stroke} strokeWidth={selected ? 3 : 1.2}
          ...
          fill={item.kind === 'slots' ? '#e1eee56b' : undefined} fillRule="evenodd"
      ...
      {item.lines.map((line, index) => { const points = line.flatMap(point => worldToScreen(move(point, delta), camera)); return points.every(Number.isFinite) ? <Line key={'line-' + index} points={points} stroke={stroke} strokeWidth={3}/> : null; })}
