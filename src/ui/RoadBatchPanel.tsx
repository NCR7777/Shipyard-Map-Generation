...
export function RoadBatchPanel({ map, ids, onApply, onCancel }: { map: YardMap; ids: string[]; onApply: (command: MapCommand) => boolean; onCancel: () => void }) {
  ...
  function apply() {
    ...
    if (onApply({ type: 'updateRoadBatch', ids, patch, ...(widthTouched && width.trim() ? { designAssumption: { id: sourceId, description: '明确选择连续道路后批量修改宽度。' } } : {}) })) onCancel();
  }
  return <Modal title="连续路段一起修改" onCancel={onCancel}><p>范围在路口、方向、宽度、所有者或资源条件变化处停止。只应用本次实际输入的字段。</p><ul>{ids.map(id => <li key={id}>{map.roads[id]!.name} · {id}</li>)}</ul><p>宽度现值：{widths.size > 1 ? '混合' : map.roads[ids[0]!]!.widthM.state === 'known' ? String((map.roads[ids[0]!]!.widthM as {value:number}).value) + ' m' : '未确定'}</p>
