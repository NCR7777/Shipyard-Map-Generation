...
import { normalizeSelection, selectionImpact, commandSupport, canEditBoundary, type FacilityMovePolicy, type ZoneMovePolicy, type MapCommand, type Selection } from '../domain/commands';
...
function PolygonPanel({ selected, map, readonly, onApply, onDirtyChange, facilityMovePolicy, zoneMovePolicy = 'boundaryOnly', boundaryEditMode = 'auto', onBoundaryModeChange, draftContext, onDraftChange, propertyUnits = DEFAULT_PROPERTY_UNITS, onPropertyUnitsChange, onPlaceAction }: SpatialPanelProps & { selected: Extract<SpatialSelection, { kind: 'facility' | 'zone' }> }) {
  ...
  const [angleUnit, setAngleUnit] = useState(propertyUnits.angle);
  ...
  function transform(type: 'translateSelection' | 'rotateSelection'): boolean {
    ...
    if (!vector || !angle.trim() || !Number.isFinite(Number(angle))) { setError('平移/旋转参数必须是有限数值；请检查旋转显示单位。'); return false; }
    const command: MapCommand = type === 'translateSelection'
      ? { type, selection, delta: vector, facilityMovePolicy, zoneMovePolicy }
      : { type, selection, pivot: vector, angleRad: Number(angle) * unitFactor(angleUnit), facilityMovePolicy, zoneMovePolicy };
    const accepted = onApply(command);
  ...
  return <div className="property-content spatial-property">
    ...
    {selected.kind === 'zone' && <p className="field-note">显式归属作业点 {zoneServicePointIds(map, selected.id).length} 个；区域覆盖不表示拥有对象。</p>}
    {onPlaceAction && <div className="tool-grid"><button disabled={readonly || basePending || moving || rotating} onClick={() => onPlaceAction(selected.kind === 'facility' ? 'facilities' : 'zones', selected.id, 'enter')}>编辑内部</button>{selected.kind === 'facility' && <button disabled={readonly || basePending || moving || rotating} onClick={() => onPlaceAction('facilities', selected.id, 'access')}>添加入口</button>}<button disabled={readonly || basePending || moving || rotating} onClick={() => onPlaceAction(selected.kind === 'facility' ? 'facilities' : 'zones', selected.id, 'service')}>添加作业点</button></div>}
    <p className="field-note">边界尺寸或顶点调整始终只改边界。已有入口和服务点保留原坐标，请核对它们与新边界的空间关系；不会通过移动关联点掩盖问题。</p>
    <details><summary>整体移动影响与技术详情</summary><label className="field-label">稳定 ID<input aria-label="稳定 ID" readOnly value={selected.id}/></label><p className="field-note" data-testid="spatial-move-impact">{!support.allowed && support.issues.map(issue => issue.message).join("；")} 整体移动/旋转策略将移动 {impact.selection.nodes.length} 个节点；关联道路 {impact.affectedRoadIds.join('、') || '无'}。共享节点 {impact.sharedNodeIds.join('、') || '无'}；引用相同节点的入口/服务点会同步。</p></details>
    {boundaryReadonly && <p className="field-note">此边界有联动依赖，禁止单独改尺寸或顶点；请使用批准的整体联动策略。</p>}
    {rectangle && <label className="field-label">边界编辑模式<select aria-label="边界编辑模式" disabled={boundaryReadonly} value={boundaryEditMode} onChange={event => onBoundaryModeChange?.(event.target.value as 'auto' | 'polygon')}><option value="auto">矩形约束 · 宽高独立</option><option value="polygon">自由多边形 · 允许改变直角</option></select></label>}
    {rectangular ? <section className="rectangle-editor">
      ...
      <div className="coordinate-fields">{(['宽', '高'] as const).map((label, index) => <label className="field-label" key={label}>{label} (m)<input aria-label={'矩形' + label + ' (m)'} type="number" step="any" min={MIN_RECTANGLE_SIZE_M} disabled={boundaryReadonly} value={dimensions[index]} onChange={event => setDimensions(current => current.map((value, i) => i === index ? event.target.value : value))} /></label>)}</div>
      <label className="field-label">固定对角<select aria-label="固定对角" disabled={boundaryReadonly} value={fixedCorner} onChange={event => setFixedCorner(Number(event.target.value) as RectangleCorner)}>{rectangle.corners.map((point, index) => <option key={index} value={index}>角 {index + 1} · {point.map(value => Number(value.toPrecision(8))).join(', ')} m</option>)}</select></label>
    </section> : <details><summary>边界顶点 · m</summary><p className="field-note">自由多边形编辑，不保持直角。外环逆时针、孔洞顺时针。闭合点自动等于首点；编辑不反转顶点数组。</p>
      {rings.map((ring, ringIndex) => <section key={ringIndex} className="ring-editor"><strong>{ringIndex === 0 ? '外环' : `孔洞 ${ringIndex}`}</strong>
        {ring.map((point, pointIndex) => <div key={pointIndex} className="spatial-vertex"><span>{pointIndex + 1}</span>{(['X', 'Y', 'Z'] as const).map((axis, axisIndex) => <input key={axis} type="number" step="any" disabled={boundaryReadonly} aria-label={`${ringIndex === 0 ? '外环' : `孔洞 ${ringIndex}`} 顶点 ${pointIndex + 1} ${axis} (m)`} value={point[axisIndex] ?? ''} onChange={event => setRings(current => current.map((r, ri) => ri === ringIndex ? r.map((p, pi) => pi === pointIndex ? p.map((value, ai) => ai === axisIndex ? event.target.value : value) : p) : r))} />)}<button disabled={boundaryReadonly || ring.length <= 3} aria-label={`删除${ringIndex === 0 ? '外环' : `孔洞 ${ringIndex}`}顶点 ${pointIndex + 1}`} onClick={() => setRings(current => current.map((r, ri) => ri === ringIndex ? r.filter((_, pi) => pi !== pointIndex) : r))}>×</button></div>)}
        <button disabled={boundaryReadonly} onClick={() => addVertex(ringIndex)}>添加{ringIndex === 0 ? '外环' : `孔洞 ${ringIndex}`}顶点</button>
        {ringIndex > 0 && <button disabled={boundaryReadonly} onClick={() => setRings(current => current.filter((_, ri) => ri !== ringIndex))}>删除孔洞 {ringIndex}</button>}
      </section>)}
      <button disabled={boundaryReadonly} onClick={addHole}>添加孔洞草稿</button><p className="field-note">孔洞草稿放在包围盒中部；提交时检查内含、重叠和相交，不保证初始草稿适合凹多边形。</p>
    </details>}
    ...
    <details><summary>数值平移与旋转</summary>
      ...
      <label className="field-label">角度显示单位<select aria-label="角度显示单位" value={propertyUnits.angle} disabled={!onPropertyUnitsChange || rotating} onChange={e => { const next = e.target.value as PropertyUnits['angle']; setAngleUnit(next); onPropertyUnitsChange?.({ ...propertyUnits, angle: next }); }}><option value="deg">度</option><option value="rad">rad</option></select></label>
      <label className="field-label">旋转角 ({rotating ? angleUnit : propertyUnits.angle})<input disabled={readonly} type="number" step="any" aria-label={'旋转角 (' + (rotating ? angleUnit : propertyUnits.angle) + ')'} value={angle} onChange={event => { setAngleUnit(propertyUnits.angle); setAngle(event.target.value); }} /></label>
    ...
    <details className="provenance-note"><summary>来源与归属</summary>{selected.value.provenance.category}。边界不等于车辆入口，面积不自动变成资源容量。</details>
  </div>;
}
