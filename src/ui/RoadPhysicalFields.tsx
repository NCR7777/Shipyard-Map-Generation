...
export function RoadPhysicalFields({ draft, onChange, readonly, sources }: {
  ...
  return <details className="physical-fields">
    <summary>物理参数与来源</summary>
    <p className="field-note">unknown、无限制和不适用不是零。没有来源的新数值将明确登记为设计假设；不是实测值。</p>
    {physicalFields.map(field => <fieldset key={field} disabled={readonly}>
