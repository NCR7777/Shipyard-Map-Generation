...
import { buildConnectedPointCreation } from './PointCreationPanel';
...
export function ConnectedPointFields({ draft, map, readonly, issues, onChange, onCreate, onCancel }: {
  ...
  const proxy = draft.kind === 'servicePoints' && draft.arrival.mode === 'node_proxy';
  ...
  <div className="dialog-actions"><button data-cancel onClick={onCancel}>取消</button><button className="primary-button" disabled={readonly} onClick={onCreate}>确认关联并创建</button></div></Modal>;
}
