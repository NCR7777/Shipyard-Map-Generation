...
export interface DisplayEntry {
  key: string; kind: SceneKind; id: string; name?: string;
...
export function createDisplayIndex(scene: SceneSnapshot): DisplayIndex {
  ...
  const entries = scene.items.filter(item => geometryKinds.has(item.kind)).map((item): DisplayEntry => {
    ...
    return { key: item.key, kind: item.kind, id: item.id, name: item.name,
...
/** Names first (IDs stay in the inspector); long text is shortened for display only. */
function labelText(entry: DisplayEntry): string {
  const text = entry.name?.trim() || (entry.kind === 'slots' ? entry.id.split(':').at(-1)! : entry.id);
