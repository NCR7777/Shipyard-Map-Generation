...
- **rasterio 从哪里来**：本工具的 `.tools/TIF-tools`（用户 2026-09-25 决定从 `../map/.cache/TIF-tools` 移来；128 MiB，不纳入 git）里单独安装的 rasterio 1.4.4 / GDAL 3.10.3，配 `paper` 的 Python。`prepare_tiffs.py` 用 `--tools` 参数指向它（路径不写死）：脚本在 `projects/TIF_可导入底图_20260915/_processing/` 下，`--tools` 要给 TIF-tools 的绝对路径，或从本工具目录运行时给 `.tools/TIF-tools`。注意它自己用 `sys.path.insert(0, …)` 把 TIF-tools 放在最前面，实际用的是 TIF-tools 里的 NumPy 2.4.6（这是那批交付的原样做法，保持不改）。本脚本的 GeoTIFF 自动读取同样需要 rasterio：把 TIF-tools 加到 `sys.path` 末尾即可，不必安装，例如在本工具目录 `conda run --no-capture-output -n paper python -c "import runpy, sys; sys.path.append(r'.tools\TIF-tools'); sys.argv = ['scripts/BG01_prepare_raster.py', '--image', 'IN.tif', '--map', 'MAP.json', '--extent', 'XMIN', 'YMIN', 'XMAX', 'YMAX', '--mpp', '1', '--out', 'NEW_DIR']; runpy.run_path(sys.argv[0], run_name='__main__')"`。不要用 `PYTHONPATH`：它把目录插到最前面，TIF-tools 里的 NumPy（2.4.6）会盖过 `paper` 自带的 2.4.4，这种写法没有验证过；审查时用这种方式跑通过一张真实的威海 GeoTIFF，输出与交付的底图对齐（平均绝对误差 2.36 个灰度级，相关系数 0.995，无平移）。下文的 `conda install -c conda-forge` 会在 `paper` 里混用 conda 通道，可能改动其他包，安装前先确认。
